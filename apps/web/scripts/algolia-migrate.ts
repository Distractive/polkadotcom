...
const PAGE_INDEX_NAME = 'page';
const BLOG_INDEX_NAME = 'blog';
const CASE_STUDIES_INDEX_NAME = 'case-studies';
const NEWSROOM_INDEX_NAME = 'newsroom';
...
const { algoliaAppId, algoliaApiKey, sanityProjectId, sanityDataset } =
  getConfig();
...
async function migrate(indexName: string) {
  ...
  try {
    ...
    if (indexName === BLOG_INDEX_NAME) {
      records = records.filter((record) =>
        record.slug.includes(BLOG_INDEX_NAME),
      );
    } else if (indexName === CASE_STUDIES_INDEX_NAME) {
      records = records.filter((record) =>
        record.slug.includes(CASE_STUDIES_INDEX_NAME),
      );
    } else if (indexName === NEWSROOM_INDEX_NAME) {
      records = records.filter((record) =>
        record.slug.includes(NEWSROOM_INDEX_NAME),
      );
    } else {
...
async function main() {
  await migrate(PAGE_INDEX_NAME);
  await migrate(BLOG_INDEX_NAME);
  await migrate(CASE_STUDIES_INDEX_NAME);
  await migrate(NEWSROOM_INDEX_NAME);
}
