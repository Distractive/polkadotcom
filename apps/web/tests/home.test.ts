...
test('Homepage', async ({ page }) => {
  ...
  await test.step('assert "hero" section is displayed properly', async () => {
    ...
    await expect(section.getByText('Polkadot is the most')).toHaveText(
      'Polkadot is the most scalable, secure, and resilient blockchain platform—designed to bring Web3 to the world.',
    );
    await expect(
  ...
  await test.step('assert "Power without permission" section displayed properly', async () => {
    ...
    await expect(section.getByText('Every protocol')).toHaveText(
  ...
  await test.step('assert "ecosystem" section is displayed properly', async () => {
    ...
    await expect(
      section.getByRole('heading', {
        name: 'Where ideas scale globally',
      }),
    ).toBeVisible();
    await expect(section.getByText('Polkadot gives builders')).toHaveText(
      'Polkadot gives builders the decentralized tools and community to turn breakthrough ideas into real-world impact.',
    );
    await expect(
      section.getByRole('link', {
        name: 'FIFA Rivals',
