// Auto-generated metrics from build process
// Generated on: 2025-07-30T22:06:30.845Z

export const staticMetricsStore = {
  getActiveValidators: '600',
  getApprovedReferendums: 943,
  getAverageMonthlyGovernanceVoters: 15661,
  getDailyTreasuryBalanceUSD: 70665749.82,
  getDaoMembers: '1.49 million',
  getPercentDOTSupplyStaked: '53%',
  getPolkadotUptime30d: '100%',
  getTotalDOTStaked: '836614566.3352035',
  getTotalFeesUSD30d: 3828.5388931739644,
  getTotalNominators: 31377,
  getTotalReferendums: '1,685 referenda',
  getTotalStablecoinsUSD: 56138777.315139845,
  getTotalStakers: 68522,
  getTotalUniqueAccounts: 8470124,
};
